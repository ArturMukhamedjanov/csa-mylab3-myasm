...
# Работа с бинарным представлением:

def get_data_bin_value(instr):
    data = "0"
    value = instr["value"]
    if isinstance(value, int):
        data += "0"
        binary_value = format(value, 'b')
        binary_value_30bit = binary_value.zfill(30)
        data += binary_value_30bit
    if isinstance(value, str):
        data += "1"
        ascii_value = ord(value)
        binary_value = format(ascii_value, 'b')
        binary_value_30bit = binary_value.zfill(30)
        data += binary_value_30bit
    return data

def get_binary_adress(instr):
    index = instr["index"]
    ind_binary_value = format(index, 'b')
    ind_binary_value_32bit = ind_binary_value.zfill(32)
    return ind_binary_value_32bit

def get_command_bin_value(instr):
    data = "1"
    if(instr["opcode"] in op_commands):
        data += "1"
        if(instr["address"]):
            data += "1"
        else:
            data += "0"
        data += opname_opcode_mapping[instr["opcode"]]
        operand_binary = format(instr["operand"], 'b').zfill(11)
        data += operand_binary
        data += "0" * 13
    else:
        data += "00"
...
def get_binary_code(code):
    ...
    debug = []
    for instr in code:
        adress = get_binary_adress(instr)
        data = ''
        if(instr["opcode"] == "nop"):
            data = get_data_bin_value(instr)
            binary.append((adress, data))
            debug.append((instr["index"], data, "value: " + str(instr["value"])))
            continue
        else:
            data = get_command_bin_value(instr)
            if(instr["opcode"] in op_commands):
                debug.append((instr["index"], data, "mnemonic: " + str(instr["opcode"]) + " operand: " +str(instr["operand"])))
            else:
                debug.append((instr["index"], data, "mnemonic: " + str(instr["opcode"])))
            binary.append((adress, data))
    return binary, debug
...
def main(code_source, code_target):
    ...
    binary, debug = get_binary_code(code)
    write_binary_code(code_target, start_address, binary)
    write_debug_code(code_target+ ".debug", debug)
    global labels
    labels = {}
